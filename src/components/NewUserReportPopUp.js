...
const NewUserReportPopUp = (props) => {
  ...
  const ReportHandlerChange = (e) => {
    ...
    axios
      .post(
        `${requestURL}/user/${props.uid ? props.uid : ''}/user-post/${props.userPostId ? props.userPostId : ''}/report`, reportData)
      .then((res) => {
        if (res.data.message) {
...
export default NewUserReportPopUp;
