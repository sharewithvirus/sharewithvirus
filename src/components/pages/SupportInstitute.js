...
const SupportInstitute = () => {
  ...
  const [insSupport, setInsSupport] = useState([]);
  ...
  useEffect(() => {
    ...
    axios.get(`${requestURL}/all/ins/support`).then((res) => {
      setInsSupport(res.data.support);
    });
  }, []);
                ...
                <div className={` ${styles.outer2}`}>
                  <form className="row">
                    ...
                    <div className="row d-flex justify-content-between align-items-center mb-4">
                      ...
                      >
                        <p>{insSupport.length}</p>
                        <p>Action Pending</p>
                      </div>
                    </div>
                    {insSupport && insSupport.map((ct) => (
            ...
            <div className="col-xl-9 col-lg-8 col-md-12 d-flex justify-content-between align-items-center">
              <div>
                <img
                  ...
                  src={
                    ct.institute.photoId === "1"
                      ? "/images/image-boy2.png"
                      : `${requestURL}/userprofileabout/coverphoto/${ct.institute.profilePhoto}`
                  }
                  alt="Profile"
                />
                <span className="mt-3 mx-3">
                   <Link to='#'>{ct.institute.name}</Link>
                </span>
...
export default SupportInstitute;
