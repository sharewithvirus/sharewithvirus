import styles from "../../Home.module.css";
import "bootstrap/dist/css/bootstrap.min.css";
import NavbarTopUser from "../../NavbarTopUser";
import NavbarBottomUser from "../../NavbarBottomUser";
import SideBarUserWarpper from "./SideBarUserWrapper";
import { useParams } from "react-router-dom";
import React, { useState, useEffect } from "react";
import "bootstrap/dist/css/bootstrap.min.css";
import axios from "axios";
import { requestURL } from "../../ReqUrl";
import EcontentInfoForm from "./EcontentInfoForm";
import EContentCart from "../pages/EContentCart";
import EContentOperator from "../pages/EContentOperator";
import PlaylistImageUpload from "./PlaylistImageUpload";
const StaffWrapper = () => {
  const params = useParams();
  const [data, setData] = useState("");
  const [first, setFirst] = useState(false);
  const [info, setInfo] = useState(false);
  const [playlist, setPlaylist] = useState(true);
  const [cart, setCart] = useState(false);
  const [runEffect, setRunEffect] = useState(true);
  useEffect(() => {
    axios
      .get(`${requestURL}/e-content/${params.eid}`)
      .then((res) => {
        setData(res.data.elearning);
        setFirst(true);
        setRunEffect(false);
      })
      .catch(() => {
        console.log("Something went wrong");
      });
  }, [runEffect]);

  return (
    <>
      <NavbarTopUser uid={params.id} />
      <div className={styles.mainScreen}>
        <div className={`${styles.mainContent} ${styles.dashboard}`}>
          <div className="row">
            <div className={`col col-lg-4 col-xl-3`}>
              <SideBarUserWarpper sid={params.sid} />
            </div>
            <div className={`col col-lg-8 col-xl-9  mx-auto ${styles.midside}`}>
              <div className={`${styles.about}`} style={{ marginTop: "24px" }}>
                <div
                  className={` ${styles.outer2} ${styles.profileCreationPage}`}
                >
                  <PlaylistImageUpload
                    eid={params.eid}
                    profileImg={
                      data.photoId === "1"
                        ? "/images/department-avatar.jpeg"
                        : first
                        ? `${requestURL}/e-content/${data._id}/${data.photo}`
                        : null
                    }
                    coverImg={
                      data.coverId === "2"
                        ? "/images/other-places-cover-photo.jpg"
                        : first
                        ? `${requestURL}/e-content/${data._id}/${data.cover}`
                        : null
                    }
                  />
                  <div className="row" style={{ marginTop: "-9rem" }}>
                    <div className="col-4" style={{ marginTop: "-2rem" }}>
                      <p>{data.playlist ? data.playlist.length : 0}</p>
                      <p>Total Playlists</p>
                    </div>
                    <div className="col-4" style={{ marginTop: "10px" }}>
                      <h4>Learnings</h4>
                    </div>
                    <div className="col-4" style={{ marginTop: "-2rem" }}>
                      {data.about
                        ? data.about.length > 100
                          ? ` ${data.about.substr(0, 100)}...`
                          : data.about.substr(0, 100)
                        : "Department About ..."}
                    </div>
                  </div>
                  <div className={`${styles.ddetail}`}>
                    <div className="row">
                      <div className="col-4">
                        <div className={`${styles.dTab}`}>
                          <span
                            onClick={() => {
                              setPlaylist(true);
                              setCart(false);
                              setInfo(false);
                            }}
                          >
                            <img
                              src="/images/video-play-icon.svg"
                              alt="user"
                              data-toggle="tooltip"
                              data-placement="bottom"
                              title="Play"
                            />
                          </span>
                        </div>
                      </div>
                      <div className="col-4">
                        <div className={`${styles.dTab} ${styles.active}`}>
                          <span
                            onClick={() => {
                              setInfo(true);
                              setPlaylist(false);
                              setCart(false);
                            }}
                          >
                            <img
                              src="/images/info-circle-icon.svg"
                              alt="user"
                              data-toggle="tooltip"
                              data-placement="bottom"
                              title="Info"
                            />
                          </span>
                        </div>
                      </div>
                      <div className="col-4">
                        <div className={`${styles.dTab} ${styles.active}`}>
                          <span
                            onClick={() => {
                              setCart(true);
                              setPlaylist(false);
                              setInfo(false);
                            }}
                          >
                            <img
                              src="/images/cart-icon.svg"
                              alt="user"
                              data-toggle="tooltip"
                              data-placement="bottom"
                              title="Cart"
                            />
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div style={{ marginTop: "3.5rem", marginBottom: "1.5rem" }}>
                    {info && <EcontentInfoForm data={data} />}
                    {playlist && (
                      <EContentOperator
                        id={params.id}
                        sid={params.sid}
                        setRunEffect={setRunEffect}
                        data={data ? data.institute.classRooms : ""}
                      />
                    )}
                    {cart && <EContentCart />}
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <NavbarBottomUser />
    </>
  );
};

export default StaffWrapper;
