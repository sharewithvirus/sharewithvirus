...
import InsIDRqst from "../InsIDRqst";
import axios from "axios";
import { requestURL } from "../ReqUrl";
import AdminSideBar from "../AdminSideBar";
import AdminAbout from "./AdminAbout";

const AdminInsIdCard = () => {
  ...
  useEffect(() => {
    axios.get(`${requestURL}/admindashboard/${params.aid}`).then((res) => {
      ...
      setAdminData(data);
      setViewIns(res.data.admin.instituteIdCardBatch);
    });
  }, [adminData]);
            ...
            <div className={`col col-lg-8 col-xl-9  mx-auto ${styles.midside}`}>
              <div className={`${styles.about}`} style={{ marginTop: "22px" }}>
                <div className={` ${styles.outer2}`}>
                  <form className="row">
                    <h4>Institute Id Card Requests</h4>
